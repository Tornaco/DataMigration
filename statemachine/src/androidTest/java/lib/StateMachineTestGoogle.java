...
import org.newstand.lib.StateMachine.LogRec;
 ...
 */
@Suppress // Failing
public class StateMachineTestGoogle extends TestCase {
    ...
    private void dumpLogRecs(StateMachine sm) {
        ...
        for (int i = 0; i < size; i++) {
            StateMachine.LogRec lr = sm.getLogRec(i);
            tlog(lr.toString());
        }
    }

    private void dumpLogRecs(Collection<StateMachine.LogRec> clr) {
        int size = clr.size();
        tlog("size=" + size);
        for (StateMachine.LogRec lr : clr) {
            tlog(lr.toString());
        }
    }
