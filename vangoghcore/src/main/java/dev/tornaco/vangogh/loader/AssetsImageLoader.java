...
public class AssetsImageLoader extends BaseImageLoader {
    ...
    Image doLoad(@NonNull ImageSource source, @Nullable LoaderObserver observer) {
        ...
        String tmpImagePath = getContext().getCacheDir().getPath() + File.separator
                + UUID.randomUUID().toString();
        if (!copyFromAssetsTo(path, tmpImagePath))
            return null;
    ...
    private boolean copyFromAssetsTo(String from, String to) {
        ...
        try {
            ...
            return copy(openAssetsInput(from), fos) > 0;
        } catch (IOException e) {
    ...
    private InputStream openAssetsInput(String fileName) throws IOException {
        return openAsset().open(fileName);
    }

    private AssetManager openAsset() {
