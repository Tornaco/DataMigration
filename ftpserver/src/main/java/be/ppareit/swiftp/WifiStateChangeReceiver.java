...
public class WifiStateChangeReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (info == null) {
            Logger.e("Null network info received, bailing");
            return;
        }
        if (info.isConnected()) {
            Logger.d("We are connecting to a wifi network");
            Intent startServerIntent = new Intent(context, StartServerService.class);
            context.startService(startServerIntent);
        } else {
            Logger.d("We are disconnected from wifi network");
    ...
    static public class StartServerService extends IntentService {
        ...
        protected void onHandleIntent(Intent intent) {
            if (FsService.isRunning()) {
                Logger.v("We are connecting to a new wifi network on a running server, ignore");
                return;
            }
            WifiManager wifiManager = (WifiManager) getApplicationContext().getSystemService(Context.WIFI_SERVICE);
            WifiInfo wifiInfo = wifiManager.getConnectionInfo();
            if (wifiInfo == null) {
                Logger.e("Null wifi info received, bailing");
                return;
            }
            Logger.d("We are connected to " + wifiInfo.getSSID());
    ...
    static public class StopServerService extends IntentService {
        ...
        protected void onHandleIntent(Intent intent) {
            ...
            Logger.d("Wifi connection disconnected and no longer connecting, stopping the ftp server");
