...
public class SessionThread extends Thread {
    ...
    public boolean sendViaDataSocket(String string) {
        try {
            ...
            Logger.d("Using data connection encoding: " + encoding);
            return sendViaDataSocket(bytes, 0, bytes.length);
        } catch (UnsupportedEncodingException e) {
            Logger.e("Unsupported encoding for data socket send");
            return false;
        }
    }
    ...
    public boolean sendViaDataSocket(byte[] bytes, int start, int len) {
        ...
        if (dataOutputStream == null) {
            Logger.i("Can't send via null dataOutputStream");
            return false;
        }
        if (len == 0) {
        ...
        try {
            ...
            Logger.e("Couldn't write output stream for data socket, error:" + e.toString());
            return false;
        }
    ...
    public int receiveFromDataSocket(byte[] buf) {
        ...
        if (dataSocket == null) {
            Logger.i("Can't receive from null dataSocket");
            return -2;
        }
        if (!dataSocket.isConnected()) {
            Logger.i("Can't receive from unconnected socket");
            return -2;
        }
        InputStream in;
        try {
            ...
            Logger.i("Error reading data socket");
            return 0;
        }
        return bytesRead;
    }
    ...
    public boolean openDataSocket() {
        try {
            ...
            if (dataSocket == null) {
                Logger.i("dataSocketFactory.onTransfer() returned null");
                return false;
            }
            dataOutputStream = dataSocket.getOutputStream();
            return true;
        } catch (IOException e) {
            Logger.i("IOException getting OutputStream for data socket");
            dataSocket = null;
            return false;
        }
    }
    ...
    public void quit() {
        Logger.d("SessionThread told to quit");
        closeSocket();
    }

    @Override
    public void run() {
        Logger.i("SessionThread started");
        ...
        try {
            ...
            while (true) {
                ...
                if (line != null) {
                    Logger.d("Received line from client: " + line);
                    FtpCmd.dispatchCommand(this, line);
                } else {
                    Logger.i("readLine gave null, quitting");
                    break;
                }
            }
        } catch (IOException e) {
            Logger.i("Connection was dropped");
        }
        closeSocket();
    }
    ...
    public void writeBytes(byte[] bytes) {
        try {
            ...
            Logger.i("Exception writing socket");
            closeSocket();
        }
    }
    ...
    public void writeString(String str) {
        ...
        try {
            ...
            Logger.e("Unsupported encoding: " + encoding);
    ...
    public void authAttempt(boolean authenticated) {
        if (authenticated) {
            Logger.i("Authentication complete");
            userAuthenticated = true;
        } else {
            authFails++;
            Logger.i("Auth failed: " + authFails + "/" + MAX_AUTH_FAILS);
            if (authFails > MAX_AUTH_FAILS) {
                Logger.i("Too many auth fails, quitting session");
                quit();
            }
        }
    }
    ...
    public void setWorkingDir(File workingDir) {
        try {
            ...
            Logger.i("SessionThread canonical error");
        }
    }
